                                ...
                                <tr key={r.id}>
                                    ...
                                    <td>
                                        <div className="action-cell">
                                            {r.status === 'pending' && (
                                                <>
                                                    <button className="btn btn-sm btn-primary" onClick={() => handleStatusUpdate(r.id, 'reviewed')}>Review</button>
                                                    <button className="btn btn-sm btn-danger" onClick={() => handleStatusUpdate(r.id, 'dismissed')}>Dismiss</button>
                                                </>
                                            )}
                                            {r.status === 'reviewed' && (
                                                <>
                                                    <button className="btn btn-sm btn-success" onClick={() => handleStatusUpdate(r.id, 'resolved')}>Resolve</button>
                                                    <button className="btn btn-sm btn-danger" onClick={() => handleStatusUpdate(r.id, 'dismissed')}>Dismiss</button>
                                                </>
                                            )}
                                        </div>
